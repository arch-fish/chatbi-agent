...
python eval_multi.py            # 三系统 × 3 次 多指标评测（准确率/延迟/token/成本）
```

## 评测结果

**三系统受控对比**（36 题 × 3 次，DeepSeek-V4-Flash；`eval_multi.py`）：
① 单次 NL2SQL（纯 workflow）｜② 带确定性纠错的 workflow｜③ Agent（自主编排）

| 系统 | 准确率(均值±std) | 平均LLM调用 | 平均token | P95延迟 | 平均成本 |
|---|---:|---:|---:|---:|---:|
| ① 单次 NL2SQL | 69% ± 1% | 1.0 | 1239 | 13.2s | ¥0.0016 |
| ② 纠错 workflow | 67% ± 2% | 1.1 | 1419 | 21.0s | ¥0.0018 |
| ③ Agent | **87% ± 1%** | 2.6 | 5966 | 13.3s | ¥0.0064 |

**三点结论：**
1. **②≈①（都约 68%）** → agent 的收益**不是**来自“多试一次纠错”。
2. **③ 的 +19pt 全部来自 ①② 结构上做不到的四类**：相关性（SQL 无 SQRT 函数必败）、不可答识别、歧义澄清、缺失说明。
3. **但纯 SQL 查询上 agent 反而略逊**（如 hard 类 67% vs 100%）——多步与爱澄清在简单/复杂 SQL 上会帮倒忙。**结论：应做意图路由，简单查询走轻量 workflow，需要新能力才进 agent。**

> 代价量化：agent token/成本约为单次 NL2SQL 的 **~4 倍**。
> 评测方法反思：group 类一度只有 33%，用可观测逐题排查发现是**答案被 max_tokens 截断 + 启发式判分对表格格式过敏**，非 agent 之错——评测器本身也有偏差，更严谨应上 LLM-as-judge。
