...
def explain_metric(term: str) -> str:
    """回答'这个指标怎么算的/什么定义/含不含X/左右还是平均/缺失怎么处理'这类查口径问题(不是查数值)。"""
    return raw.explain_metric(term)

TOOLKIT = [query_db, lookup_columns, compute_correlation, ask_user, explain_metric]
...
- 拿到结果后用简洁中文回答，带具体数字。
- 回答尽量紧凑，避免冗长 Markdown 表格；分组/多项结果每组一行简洁列出（如“haste: 4.69；trufi: 4.61”），确保所有分组都完整输出、不被截断。"""

# few-shot 示范：教模型“遇到口径不明的统计问题，先 ask_user 反问”。
# 用“华西有多少数据”这个结构相似但不同的例子，让它学到【模式】而非死记某句。
FEWSHOT = [
    HumanMessage("某中心有多少数据"),
    ...
    AIMessage(content="", tool_calls=[{"name": "query_db",
        "args": {"sql": "SELECT COUNT(*) FROM subjects WHERE center='某中心'"}, "id": "fs2"}]),
    ToolMessage(content="128", tool_call_id="fs2"),
    AIMessage(content="某中心按个体共 128 个（含全部记录）。"),
]
...
def _make_llm():
    ...
    return ChatOpenAI(model=config.MODEL, base_url=config.BASE_URL,
                      api_key=config.get_api_key(), temperature=0,
                      max_tokens=1500).bind_tools(TOOLKIT)  # 调大，防分组等长答案被截断
...
_TOOL_BY_NAME = {t.name: t for t in TOOLKIT}

def _prior_sigs(msgs):
    """收集本轮已发生的(工具名,参数)签名，用于重复检测；跳过 few-shot(无 usage_metadata)。"""
    sigs = set()
    for m in msgs:
        if not getattr(m, "usage_metadata", None):
            continue
        for tc in getattr(m, "tool_calls", None) or []:
            sigs.add((tc["name"], _json.dumps(tc["args"], sort_keys=True, ensure_ascii=False)))
    return sigs

def tools_node(state):
    """执行工具 + 重复调用检测：相同工具+相同参数已调过，则不再执行、提示换思路。"""
    msgs = state["messages"]
    last = msgs[-1]
    prior = _prior_sigs(msgs[:-1])
    out = []
    for tc in last.tool_calls:
        sig = (tc["name"], _json.dumps(tc["args"], sort_keys=True, ensure_ascii=False))
        if sig in prior:
            content = f"重复调用检测：已用相同参数调用过 {tc['name']}，不再重复执行。请换个思路，或基于已有结果直接回答。"
        else:
            tool = _TOOL_BY_NAME.get(tc["name"])
            content = str(tool.invoke(tc["args"])) if tool else f"未知工具 {tc['name']}"
...
def build_graph(checkpointer=None):
    ...
    g.add_node("tools", tools_node)  # 自定义节点：执行工具 + 重复调用检测        # 节点2：执行工具(预置组件，自动跑tool_calls)
